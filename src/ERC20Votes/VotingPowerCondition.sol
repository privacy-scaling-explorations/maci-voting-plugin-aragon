...
contract VotingPowerCondition is PermissionCondition {
    ...
    /// @notice Initializes the contract with the `MaciVoting` plugin address and
    ...
    /// @dev The function checks both the voting power and token balance to ensure `_who` meets the
    /// minimum voting threshold defined in the `MaciVoting` plugin. Returns `false` if the minimum
